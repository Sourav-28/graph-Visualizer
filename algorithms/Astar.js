    ...
    const top=0,
         ...
         class PriorityQueue{
            ...
            replace(c){
                const d = this.peek();
                return (
                    this._heap[top] = c, this._siftDown(), d
                );
            }
            _greater(c, d){
...
function AStar(speed) {
    const pq=new PriorityQueue((a,b)=>{
        if(a[0]==b[0]){
            let ha=Math.abs(desRow-a[1])+Math.abs(desCol-a[2]); // [f_cost, row, col]
    ...
    while(!pq.isEmpty()){
        ...
        for(let i=0;i<4;++i){
            ...
            if(!(tx<0 || tx>=rows || ty<0 || ty>=cols || Visited[tx][ty] || document.getElementById(`${tx}-${ty}`).classList.contains("black"))){
                let g_cost=Math.abs(srcRow-tx)+Math.abs(srcCol-ty);
                let disToNbr=g_cost+1;

                let h_cost=Math.abs(desRow-tx)+Math.abs(desCol-ty);
                let f_cost=g_cost+h_cost;

                if(distance[tx][ty]>disToNbr){
                    distance[tx][ty]=disToNbr;
